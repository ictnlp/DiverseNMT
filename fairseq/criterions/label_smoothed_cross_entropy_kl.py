...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss, nll_loss, prob, index, non_pad_mask, lprobs, p_topk, gt_mask = self.compute_loss(model, net_output, sample, reduce=reduce)
        ...
        loss2 = self.coef2 * self.kl_categorical(lm_logits.view(-1, lm_logits.size(-1)), nmt_logits.view(-1, nmt_logits.size(-1)), non_pad_mask, gt_mask)
        logging_output = {
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        p_topk = topk[0]
        i_topk = topk[1]       
        gt_mask = 1 - torch.eq(index.view(-1, 1), target)
        gt_mask = gt_mask.view(-1)
        return loss, nll_loss, prob, target, non_pad_mask, lprobs, i_topk, gt_mask
    ...
    def kl_categorical(self, p_logit, q_logit, non_pad_mask, gt_mask):
        non_pad_mask = non_pad_mask.view(-1)
        p = F.softmax(p_logit, dim=-1)
        _kl = torch.mean(p * (F.log_softmax(p_logit, dim=-1) - F.log_softmax(q_logit, dim=-1)), 1)[non_pad_mask.byte()]
