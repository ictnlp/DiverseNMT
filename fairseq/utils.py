...
def make_positions(tensor, padding_idx, onnx_trace=False, lm=False):
    ...
    if lm:
        mask = adddim(mask)
        res = torch.cumsum(mask, dim=1) * mask
        return res[:, 2:]
    else:
        return torch.cumsum(mask, dim=1) * mask + padding_idx

def adddim(mask):
    original = torch.ones([mask.size(0),2]).to('cuda').float()
    mask = torch.cat((original, mask.float()), 1)
