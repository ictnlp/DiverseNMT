...
class TransformerModel(FairseqModel):
    ...
    def __init__(self, encoder, decoder, lm_encoder):
        super().__init__(encoder, decoder, lm_encoder)
    ...
    def add_args(parser):
        ...
        parser.add_argument('--act-dropout', default=0.1, type=float,
                            ...
                            ' activation in FFN')
        parser.add_argument('--num-segment', type=int, metavar='N', default=0,
                            help='num segment in the input')
        parser.add_argument('--sentence-class-num', type=int, metavar='N',
                            default=0, help='number of classes for sentence'
                            ' task')
    ...
    #    return self.lm_encoder.max_positions
    ...
    def build_model(cls, args, task):
        ...
        lm_encoder = MaskedLMEncoder(args, tgt_dict, decoder_embed_tokens)
        return TransformerModel(encoder, decoder, lm_encoder)
...
class MaskedLMEncoder(FairseqEncoder):
    ...
    def forward(self, tokens, encoder_out, decoder_out, **unused):
        """
        ...
        segment_labels = None
        inner_states, sentence_rep = self.sentence_encoder(tokens, segment_labels, encoder_out, decoder_out)
        x = inner_states
        x_mixed = sentence_rep

        # project back to size of vocabulary
        if self.share_input_output_embed \
                and hasattr(self.sentence_encoder.embed_tokens, 'weight'):
            x = F.linear(x, self.sentence_encoder.embed_tokens.weight)
            x_mixed = F.linear(x_mixed, self.sentence_encoder.embed_tokens.weight)
        elif self.embed_out is not None:
            x = self.embed_out(x)
            x_mixed = self.embed_out(x_mixed)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        after = x
        original = torch.cat((original, x[:, -1:, :]), 1)
        original = original.view(-1, original.size(-1))
        after = after.contiguous().view(-1, after.size(-1))
        dist = nn.functional.pairwise_distance(original, after)
        ...
        return x, {'attn': attn, 'inner_states': inner_states}, dist
